...
| 2.5.7 Dragging movements | Manual verification, `MANUAL-CHECKLIST.md` §8.1 | **Pass (issue #20 resolved, closed 2026-07-03, commit 2c286a8).** Every drag interaction has a single-pointer-free keyboard alternative in the timed views: drag-to-create via `n` → `OnCreateAtFocusRequested`; drag-to-move via `m` → arrow keys → Enter (commit `OnEventMoved`) / Escape (cancel); drag-to-resize via `Shift+ArrowUp`/`Shift+ArrowDown` → `OnEventResized`. All paths honor the consumer's `Cancel` flag, and additive `OnKeyboardMoveRequested` / `OnKeyboardResizeRequested` callbacks surface the intent. This was unblocked once issue #19 (below) made roving-tabindex transfer real focus. |
