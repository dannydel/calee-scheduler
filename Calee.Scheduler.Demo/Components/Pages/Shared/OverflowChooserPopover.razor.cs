...
/// <summary>
/// Demo-only chooser popover for the "+N" overlap block. Lists the hidden
/// events; selecting one raises <see cref="OnEventSelected"/> so the host
/// can open the existing single-event <c>EventActionPopover</c>. Per
/// ADR-0010 the library ships no popovers — this lives entirely in the
/// demo. Anchoring/dismiss/keyboard mechanics are copied verbatim from
/// <c>EventActionPopover.razor.cs</c>.
/// </summary>
public partial class OverflowChooserPopover : ComponentBase, IAsyncDisposable
{
    [Inject] private IJSRuntime JS { get; set; } = default!;

    /// <summary>Whether the chooser is rendered.</summary>
    [Parameter] public bool IsVisible { get; set; }

    /// <summary>The hidden events to choose from.</summary>
    [Parameter] public IReadOnlyList<ICalendarEvent>? Events { get; set; }

    /// <summary>Time zone used to format event times.</summary>
    [Parameter] public TimeZoneInfo? TimeZone { get; set; }

    /// <summary>Fires with the event the user picked from the list.</summary>
    [Parameter] public EventCallback<ICalendarEvent> OnEventSelected { get; set; }

    /// <summary>Fires when the chooser dismisses without a selection.</summary>
    ...
    {
        // Transition: hidden -> visible. Schedule the anchor + focus pass for
        // the next OnAfterRenderAsync, when the popover element is in the DOM.
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (!IsVisible || !_needsAnchorAndFocus)
        {
            if (IsVisible && _positionReady && _needsFocusAfterPosition)
            {
                ...
                {
                    // Close button may not be attached yet on a slow render.
                }
            }
            return;
        }
    ...
    private IEnumerable<ICalendarEvent> SortedEvents =>
        Events is null ? Array.Empty<ICalendarEvent>() : Events.OrderBy(e => e.Start);

    private async Task HandleSelectAsync(ICalendarEvent ev)
    {
        await OnDismissed.InvokeAsync();
        await OnEventSelected.InvokeAsync(ev);
    }
